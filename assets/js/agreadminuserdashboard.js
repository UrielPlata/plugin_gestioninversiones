...
( function( $ ) {
  ...
  const user = parseInt(urlParams.get('id'));
  // console.log(cuenta);

  if(!isNaN(user) == true){
    $(".tab-agreuserdep").DataTable({
      pageLength : 25,
      lengthMenu: [[10, 12, 25, -1], [10, 12, 25, 'Todos']],
      "ajax": {
        url: url_solicitar_agresivo.ajaxurl + '?action=mostrarTablaAgrUserHistoDep',
        type: 'POST',
        data: {
          "user" : user
        },
    ...
    $(".tab-agreuserret").DataTable({
      pageLength : 25,
      lengthMenu: [[10, 12, 25, -1], [10, 12, 25, 'Todos']],
      "ajax": {
        url: url_solicitar_agresivo.ajaxurl + '?action=mostrarTablaAgrUserHistoRet',
        type: 'POST',
        data: {
          "user" : user
        },
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        if(resultado.respuesta==1){
          ...
          $(".tab-agreuserdep").DataTable().ajax.reload();
          swal(
            'Solicitud generada',
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        if(resultado.respuesta==1){
          ...
          $(".tab-agreuserret").DataTable().ajax.reload();
          swal(
            'Solicitud generada',
            'Se ha enviado una liga de confirmación de solicitud a su correo. En caso de no encontrar el correo revise su bandeja de spam.',
            'success'
